    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (l != null) {
            ...
            if (token != null) {
                //验证
                log.info("验证token");
                Map<String,Object> map=authService.verifyToken(token, request.getRemoteAddr());
                if (map!=null) {
                    login_status = true;
                    //request.getSession().setAttribute("uid",userService.);
                    request.getSession().setAttribute("uid",map.get("uid"));
//                    response.addIntHeader("uid",(int)map.get("uid"));
                }
            }
            // token更新
